...
class CurriculumVitaeExcelReport(models.Model):
    _name = 'report.curriculum_vitae.reporting_cv_standard_excel_id'
    ...
    def generate_xlsx_report(self, workbook, data, lines):
        ...
        sheet.merge_range('A1:E1', lines.standard_cv_form.name, name_format)

        cv_image = lines.standard_cv_form.image
        ...
        sheet.merge_range('B3:F3', lines.standard_cv_form.mobile)
        sheet.write('A4', _('Email'), label_format)
        sheet.merge_range('B4:F4', lines.standard_cv_form.email)
        sheet.write('A5', _('Age'), label_format)
        sheet.merge_range('B5:F5', lines.standard_cv_form.age,
                          workbook.add_format({'align': 'left'}))
        sheet.write('A6', _('Date of Birth'), label_format)
        sheet.merge_range('B6:F6', lines.standard_cv_form.date_of_birth,
                          workbook.add_format({'align': 'left', 'num_format': 'dd mmm yyyy'}))
        sheet.write('A7', _('NRC'), label_format)
        sheet.merge_range('B7:F7', lines.standard_cv_form.nrc)

        sheet.write('A8', _('Nationality'), label_format)
        sheet.merge_range('B8:F8', lines.standard_cv_form.nationality)
        sheet.write('A9', _('Religion'), label_format)
        sheet.merge_range('B9:F9', lines.standard_cv_form.religion)
        sheet.write('A10', _('Gender'), label_format)
        sheet.merge_range('B10:F10', lines.standard_cv_form.gender)
        sheet.write('A11', _('Current Address'), label_format)
        sheet.merge_range('B11:F11', lines.standard_cv_form.current_address)
        sheet.write('A12', _('Marital Status'), label_format)
        sheet.merge_range('B12:F12', lines.standard_cv_form.marital_status)

        sheet.merge_range('A13:F13', _('Career Objectives'), header_format)
        sheet.set_row(13, 70)
        sheet.merge_range('A14:F14', lines.standard_cv_form.objectives,
        ...
        row += 1
        column = 0
        for ebl in lines.standard_cv_form.education_background_lines:
        ...
        eh_row += 1
        eh_column = 0
        for ehl in lines.standard_cv_form.employment_history_lines:
        ...
        pe_row += 1
        pe_column = 0
        for pel in lines.standard_cv_form.project_experiences_lines:
        ...
        ls_row += 1
        ls_column = 0
        for lsl in lines.standard_cv_form.language_skills_lines:
